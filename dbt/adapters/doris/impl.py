...
from dbt.adapters.doris import DorisConnectionManager
from dbt.adapters.doris import DorisRelation
from dbt.adapters.doris import DorisColumn
...
class DorisAdapter(SQLAdapter):
    ...
    def list_relations_without_caching(
        ...
        try:
            ...
            if f"Doris database '{schema_relation}' not found" in errmsg:
                return []
            else:
        ...
        for row in results:
            if len(row) != 4:
                raise dbt.exceptions.RuntimeException(
                    f'Invalid value from "doris__list_relations_without_caching({kwargs})", '
    ...
    def get_columns_in_relation(self, relation: Relation) -> List[DorisColumn]:
        rows: List[agate.Row] = super().get_columns_in_relation(relation)
        return self.parse_show_columns(relation, rows)

    def _get_columns_for_catalog(
        self, relation: DorisRelation
    ) -> Iterable[Dict[str, Any]]:
        columns = self.get_columns_in_relation(relation)

        for column in columns:
            # convert DorisColumns into catalog dicts
    ...
    def get_rows_different_sql(
        self,
        relation_a: DorisRelation,
        relation_b: DorisRelation,
        ...
        # Doris doesn't have an EXCEPT or MINUS operator, so we need to simulate it
        COLUMNS_EQUAL_SQL = '''
        WITH
        a_except_b as (
            SELECT
